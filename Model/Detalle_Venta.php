...
class  Detalle_Venta{
    ...
    public static function DiezLibrosMasVendidos(){
        $conexion = proyectoBD::connectDB();
        $seleccion="SELECT d.id_libro,SUM(d.cantidad) AS cantidad,l.imagen,l.titulo,l.descripcion,l.precio,l.autor,l.edicion,l.cantidadvender,l.cantidadalquiler,l.genero FROM detalle_venta d INNER JOIN libros l ON d.id_libro=l.id GROUP BY d.id_libro ORDER BY SUM(d.cantidad) DESC LIMIT 0,10";
        $consulta=$conexion->query($seleccion);
         while ($registro = $consulta->fetchObject()) {
